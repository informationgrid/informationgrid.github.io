...
| service.alternateLanguage                  | en | Wird vom Service ein **einzelner(!)** Deskriptor angefragt (getTerm), so kann der Deskriptor-Name in einer zweiten Sprache geliefert werden. Wird hier z.B. "en" eingetragen, so wird der Deskriptor immer auch in Englisch geliefert und kann im Katalog abgelegt werden. Beim Mapping nach ISO XML wird das Schlagwort dann auch zusätzlich in Englisch lokalisiert.<br>Wird hier nichts eingetragen, so wird der Deskriptor nur in der Basissprache geholt und ohne zusätzliche Lokalisierung nach ISO abgebildet.<br><br>Sollte im IGE Frontend auf "**en**" (damit englische Lokalisierung des Deskriptors nach ISO abgebildet werden kann) und im SNS-iPlug auf "" gesetzt werden (keine englische Lokalisierung nötig für "Ähnliche Begriffe"). |
